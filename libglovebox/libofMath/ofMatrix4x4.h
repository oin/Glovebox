...
#ifndef OFMATRIX4X4_H_7MFODKN7
#define OFMATRIX4X4_H_7MFODKN7
...
public:
	...
	matrix_decomposition decomposeMatrix() const;
	void recomposeMatrix(ofVec3f, ofQuaternion);	
...
//	bool operator != (const ofMatrix4x4& m) const { return compare(m)!=0; }
	...
	float getCoef(int row, int col) const {
		return _mat[row][col];
	}

	void setCoef(int row, int col, float x) {
		_mat[row][col] = x;
	}
...
#endif /* end of include guard: OFMATRIX4X4_H_7MFODKN7 */
